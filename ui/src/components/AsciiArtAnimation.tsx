...
type ElavSprite = (typeof ELAV_SPRITES)[number];
...
function spriteSize(sprite: ElavSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = ELAV_SPRITES[(baseCol + baseRow) % ELAV_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = ELAV_SPRITES[Math.floor(Math.random() * ELAV_SPRITES.length)]!;
