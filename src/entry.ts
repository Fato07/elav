...
function ensureExperimentalWarningSuppressed(): boolean {
  ...
  if (
    isTruthyEnvValue(process.env.ELAV_NO_RESPAWN) ||
    isTruthyEnvValue(process.env.OPENCLAW_NO_RESPAWN)
  ) {
    return false;
  }
  if (
    isTruthyEnvValue(process.env.ELAV_NODE_OPTIONS_READY) ||
  ...
  process.env.ELAV_NODE_OPTIONS_READY = "1";
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[elav] ${parsed.error}`);
    process.exit(2);
  }
